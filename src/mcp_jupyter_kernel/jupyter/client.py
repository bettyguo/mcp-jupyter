...
See docs/recon/jupyter-api.md for the wire protocol.

Implementation note: we use httpx (async REST) and websockets (async WS)
directly, not Datalayer's jupyter-kernel-client. The original DECISIONS
preferred that library, but its public API is sync, which would force a thread
pool wrapper for every call. Building directly on httpx+websockets is ~300
lines, fully async, and easier to unit-test with respx. See DECISIONS
2026-05-13 (updated).

v1 uses the JSON WebSocket framing. Binary v1 subprotocol negotiation is
TODO — adds ~33% throughput on plot-heavy paths but isn't required for
correctness.
"""
...
class ServerSession(KernelSession):
    ...
    Notebook IDs are session IDs from /api/sessions — they uniquely tie a
    notebook path to a kernel.
    """
...
class ConcurrentModificationError(RuntimeError):
    """Raised when the notebook changed on disk between our read and write.

    Surfaced to the agent as `{error: 'conflict', last_modified: ...}` — see
    docs/tools.md cells.insert.
    """


def _url_path(p: str) -> str:
    # Jupyter contents paths are URL path segments — encode each segment.
