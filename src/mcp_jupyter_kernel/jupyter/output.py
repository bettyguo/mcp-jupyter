...
A cell is finished only when both `status:idle` on iopub AND `execute_reply`
on shell have arrived for the same parent_header.msg_id. Either alone is
unsafe: replies can arrive before the iopub flush, and idle can fire before
