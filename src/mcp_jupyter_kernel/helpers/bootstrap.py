...
Sent via execute_code(silent=True, store_history=False). Provides the
`__mjk` namespace used by inspect and kernel.list_variables. Idempotent.

Names referenced inside the `_MjkHelpers` class body must NOT start with
two underscores: Python mangles `__name` to `_MjkHelpers__name` and the
lookup silently fails. So aliases used inside the class are single-
underscore (`_mjk_sys`, `_mjk_ns`); module-scope aliases (`__mjk`,
`__mjk_json`) keep the double underscore so %whos hides them.
"""
