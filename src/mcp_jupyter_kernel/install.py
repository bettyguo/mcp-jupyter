...
Supports Claude Desktop, Cursor, and any client using the canonical
`{"mcpServers": {...}}` JSON shape. For Claude Code (the CLI), we print
the `claude mcp add` command — no file write.

All writes are atomic (tempfile → rename) and preserve existing entries.
Refuses to overwrite if the existing config isn't valid JSON.
"""
