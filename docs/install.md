...
Try opening a notebook in Lab, then ask your agent "what tools do you have for Jupyter?" The above should show up.

## Troubleshooting

### "No Jupyter token configured"

Set `JUPYTER_TOKEN` (or whatever env var you used with `--token-env`), or
pass `--token` explicitly. The token comes from Lab's startup banner or
`jupyter server list`.

### Tools don't appear after install

- Restart your MCP client. Most only read configs on launch.
- `mcp-jupyter-kernel mcp show --client <name>` tells you the config
  path. Open it; you should see your entry under `mcpServers`.

### Server mode says "could not connect"

- Verify your Jupyter URL: `curl http://localhost:8888/api?token=$JUPYTER_TOKEN`.
- Check `base_url_prefix` if you're on JupyterHub (`/user/<name>`).

### "IOPub data rate exceeded" warnings

Jupyter Server caps output rates by default. For notebooks that produce
lots of output, raise `iopub_data_rate_limit` on the server side; this
package can't change it for you.

### Windows: `ProactorEventLoop does not implement add_reader`

Cosmetic warning from ZMQ. Standalone mode switches to
`WindowsSelectorEventLoopPolicy` at startup; server mode is unaffected.

## Uninstall

Remove the entry by hand from the client's config (path via
`mcp-jupyter-kernel mcp show --client <name>`), then `pip uninstall
mcp-jupyter-kernel`. There's no `mcp uninstall` command yet.
