...
def test_mcp_install_with_token_env_warns_about_inheritance(
    ...
    """--token-env reads from os.environ at runtime, but MCP clients
    (Claude Desktop, Cursor) spawn the binary with a restricted env.
    Install should warn so the user doesn't hit silent auth failure.
    """
...
def test_mcp_install_all_exits_nonzero_when_any_target_fails(
    ...
    """`mcp install --client all` must exit non-zero if any target fails.

    Note on isolation: env-var monkeypatching is insufficient because
    Path.home() on Windows uses USERPROFILE / HOMEDRIVE, not HOME. Swap
