"""Standalone-mode tests for inspect, list_variables, plots and debug.
...
async def test_inspect_auto_on_dataframe_with_datetime_column(session) -> None:
    """inspect_auto must not crash on Timestamps / Decimals / other non-JSON
    types. to_dict(orient='records') returns objects json.dumps can't
    serialize unless we pass default=str.
    """
    from mcp_jupyter_kernel.helpers.bootstrap import helper_call
    from mcp_jupyter_kernel.tools.kernel_ import _parse_helper_output
